...
def dumpSpecifiedWindow(hwnd, wantedText=None, wantedClass=None):
    ...
    hwndChild_text = win32gui.GetWindowText(hwndChild)
    windows.append((hwndChild,hwndChild_text))
    while True:
...
def object_search(objectname):
    out = getWindowList()
    for n, hwndMain in enumerate(out):
        if objectname in hwndMain[0]:
            return hwndMain[1]
    else:
        return 0

def click(hwndEdit):
    left,up, right,down = win32gui.GetWindowRect(hwndEdit)
    x = int((left+right)/2)
    y = int((down + up) / 2)
