...
class KiwoomAPI:
    ...
    def __init__(self, kiwoom):
        ...
        self.requestManager = _RequestManager(self)
        self.requestManager.threadEvent.connect(self._tr_request_task)
        self.requestManager.start()

        self._login = False
        self.ALL_SIG = ['OnEventConnect', 'OnReceiveTrCondition',
                        'OnReceiveTrData', 'OnReceiveChejanData',
                        'OnReceiveConditionVer','OnReceiveRealCondition',
                        'OnReceiveRealData','OnReceiveMsg']
        self.CON_MET = ['_api_onEventConnect', '_api_onReceiveTrCondition',
                        '_api_onReceiveTrData','_api_onReceiveChejanData',
                        '_api_onReceiveConditionVer', '_api_onReceiveRealCondition',
                        '_api_onReceiveRealData','_api_onReceiveMsg']

    def _connect(self):
        for s,m in zip(self.ALL_SIG,self.CON_MET):
            getattr(self.kiwoom,s).connect(getattr(self,m))


    def _disconnect(self):
        for s, m in zip(self.ALL_SIG, self.CON_MET):
            if self.kiwoom.isSignalConnected(getSignal(self.kiwoom, s)):
                getattr(self.kiwoom,s).disconnect(getattr(self,m))
    ...
    def _api_onEventConnect(self, nErrCode):
        if nErrCode == 0:
            ...
            self.stock_item = dict()
            for code in codeList:
                jongmok = self.kiwoom.dynamicCall("GetMasterStockState(QString", code)
                if not ('거래정지' in jongmok) and not ('관리종목' in jongmok):
                    self.stock_item[code] = {'종목명': self.kiwoom.dynamicCall("GetMasterCodeName(QString)", code),
                ...
                self.stockItemList.append(
                    {'종목코드':code,'종목명': self.kiwoom.dynamicCall("GetMasterCodeName(QString)", code),
                     '전일가': int(self.kiwoom.dynamicCall("GetMasterLastPrice(QString)", code)),
                     '상장주식수': int(self.kiwoom.dynamicCall("GetMasterListedStockCnt(QString)", code)),
                     '상장일': self.kiwoom.dynamicCall("GetMasterListedStockDate(QString)", code),
                     '감리구분': self.kiwoom.dynamicCall("GetMasterConstruction(QString", code),
    ...
    def _api_onReceiveRealData(self, sCode, sRealType, sRealData):
        if sRealType in set(['주식체결','주식예상체결','주식시간외호가','ECN주식체결']):
            if sRealType == '주식체결':
                realData = {
                    '체결시간': self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 20),
                    '현재가': abs(int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 10))),
                    '전일대비': float(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 11)),
                    '등락률': float(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 12)),
                    '최우선매도호가': abs(int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 27))),
                    '최우선매수호가': abs(int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 28))),
                    '거래량': int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 15)),
                    '누적거래량': int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 13)),
                    '누적거래대금': float(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 14)),
                    '시가': abs(int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 16))),
                    '고가': abs(int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 17))),
                    '저가': abs(int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 18))),
                    ...
                    '시가총액': float(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 311)),
                ...
                realData = {
                    '호가시간': self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 21),
                    '시간외매도호가총잔량': int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 131)),
                    '시간외매도호가총잔량직전대비': int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 132)),
                    '시간외매수호가총잔량': int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 135)),
                    '시간외매수호가총잔량직전대비': int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 136)),
                }
            elif sRealType == '장시작시간':
                realData = {
                    '장운영구분':self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 215),
                    '체결시간': self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 20),
                    '장시작예상잔여시간': self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, 214),
                }
            elif sRealType == 'ECN주식체결':
                sRealData.split("\t")
                realData = {}
                for k,z in zip(['현재가', '전일대비', '등락율', '누적거래량', '누적거래대금', '거래량', '시가', '고가',  '저가', '체결시간','전일대비기호','매도호가','매수호가']
                ,[10010  ,  10011  , 10012  , 10013     ,        10014, 10015  , 10016, 10017, 10018, 10020, 10025, 10027, 10028]):
                    t_data = self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, z)
                    try:
                        t_data = float(t_data)
                    except:
                        pass
            ...
            for i in range(0,100000):
                k = self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, i)
                if k != '':
                    append_int.append((i,k))
            """
    ...
    def _api_onReceiveMsg(self,sScrNo, sRQName, sTrCode, sMsg):
    ...
    def addRealData(self, codeList):
        if self.getLoginState():
            ...
            #fidList += ';21;131;132;135;136'
            fidList = '9001;302;10;21;131;132;135;136'
            for code in codeList:
    ...
    def removeRealData(self, codeList):
        if self.getLoginState():
            if isinstance(codeList, str):
                codeList = [codeList]
            ret = []
            for code in codeList:
                if code not in self._realDataDict:
                    scrNum = self._realDataDict[code]
                    r = self.kiwoom.dynamicCall('SetRealRemove(QString, QString)', scrNum, code)
                    ret.append((code,r))
                    del self._realDataDict[code]
                    self._realScrNumDict[scrNum].remove(code)
        return ret
...
class _RequestManager(QThread):
    ...
    def __init__(self, parent,request_delay=650):
